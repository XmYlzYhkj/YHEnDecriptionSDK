...
@implementation YHSecKeyWrapper
/*
 * RSA 加密
 */
#pragma mark - 获取RSA加密后的字符
...
+ (NSString *)encryptUseRSA:(NSString *)content publicKey:(NSString *)publicKeyString secPadding:(SecPadding)secPadding
{
    ...
    NSData *publicKey = [NSData dataFromBase64String:publicKeyString];
    NSData *contentUTF8 = [content dataUsingEncoding: NSUTF8StringEncoding];
    
    SecKeyRef publicKeyRef = NULL;
    YHSecKeyWrapper * secKeyWrapper = [YHSecKeyWrapper sharedWrapper];
...
+ (BOOL)verifySignatureWithString:(NSString *)content signature:(NSString *)sig publicKey:(NSString *)publicKeyStr
{
    ...
    SecKeyRef publicKeyRef = NULL;
    NSData *publicKey = [NSData dataFromBase64String:publicKeyStr];
    publicKeyRef = [[YHSecKeyWrapper sharedWrapper] addPublicKey:kPublicKeyName keyBits:[[YHSecKeyWrapper sharedWrapper] stripPublicKeyHeader:publicKey]];
    if (NULL == publicKeyRef)
    {
        return NO;
    }
    NSData *contentUTF8 = [content dataUsingEncoding: NSUTF8StringEncoding];
    NSData *sigData = [NSData dataFromBase64String:sig];
    BOOL result= [[YHSecKeyWrapper sharedWrapper] verifySignature:contentUTF8 secKeyRef:publicKeyRef signature:sigData];
    [[YHSecKeyWrapper sharedWrapper] removePublicKey:kPublicKeyName];
    return result;
}
...
+ (NSString *)encryptUseAES:(NSString *)content withKey:(NSString *)key withIv:(NSString *)iv withEncodeType:(NSString *)encode
{
    ...
    NSData *encryptedData = [[YHSecKeyWrapper sharedWrapper] doAESCipher:contentData operation:kCCEncrypt key:key iv:iv];
    //加密后的数据
    NSString *result = nil;
    if (encryptedData) {
        if ([encode isEqualToString:AES_ENCODE_TYPE_BESE64]) {
            result = [encryptedData base64EncodedString];
        }else if ([encode isEqualToString:AES_ENCODE_TYPE_HEX]){
            result = [encryptedData hexadecimalString];
        }
    }
...
+ (NSData *)dataWithDecryptAESWrap:(NSString *)encryptedString withKey:(NSString *)key withIv:(NSString *)iv withEncodeType:(NSString *)encode
{
    ...
    if ([encode isEqualToString:AES_ENCODE_TYPE_BESE64]) {
        ...
        encryptedData = [NSData dataFromBase64String:encryptedString];
        
    }else if ([encode isEqualToString:AES_ENCODE_TYPE_HEX]){
        
        encryptedData = [NSData dataFromHexadecimalString:encryptedString];
    }
    //解密
    NSData *decryptedData = [[YHSecKeyWrapper sharedWrapper] doAESCipher:encryptedData operation:kCCDecrypt key:key iv:iv];
...
+ (NSString *)decryptAESWrap:(NSString *)encryptedString withKey:(NSString *)key withIv:(NSString *)iv withEncodeType:(NSString *)encode
{
    ...
    if (decryptedData) {
        ...
        if (!resultString) {
            resultString = [decryptedData base64EncodedString];
        }
    }
    return resultString;
...
static YHSecKeyWrapper * __sharedKeyWrapper = nil;
...
+ (NSString *)encryptUseAES:(NSString *)content key:(NSString *)key iv:(NSString *)iv
{
    ...
    NSData *encryptedData = [[YHSecKeyWrapper sharedWrapper] doAESCipher:contentData operation:kCCEncrypt key:key iv:iv];
    //加密后的数据
    NSString *result = encryptedData?[encryptedData base64EncodedString]:nil;
